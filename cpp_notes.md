...
- 内存对齐原则：1.从0位置开始存储；2.变量存储的起始位置是该变量大小的整数倍；3.结构体总的大小是其最大元素的整数倍，不足后面补齐；4.结构体中包含结构体，从结构体中最大元素的整数倍开始存；
...
- `weak_ptr`：是一种不控制所指向对象生存周期的智能指针，他指向由一个`shared_ptr `管理的对象，不改变`shared_ptr`的引用计数，
  ...
  - 需要一个`shared_ptr`来来初始化；
    - 最后一个shared_ptr 被销毁，对象就会被销毁，即使有`weak_ptr`指向对象；
...
- RTTI，就是利用`typeid()`判断传入的指针或引用的实际类型，然后利用`dynamic_cast()`将传入的指针或引用转换为实际类型；运行；
