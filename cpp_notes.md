...
- 内存对齐原则：1.从0位置开始存储；2.变量存储的起始位置是该变量大小的整数倍；3.结构体==总的大小==是其最大元素的整数倍，不足后面补齐；4.结构体中包含结构体，从结构体中最大元素的整数倍开始存；
...
- 静态多态：（早绑定，静态绑定），编译时就能确定的；编译时将函数的实现和函数的调用绑定起来，通过重载和模板技术实现；
...
- 使用：
  ...
  - using声明：`using std::cout; using namespace std;`,使特定命名空间（的特定成员）可见；
